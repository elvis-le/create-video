...
Dựa trên các Dữ Liệu Đầu Vào, hãy viết ngay kịch bản toàn cảnh (Master Script). Chỉ trả về nội dung kịch bản, không giải thích gì thêm.`;
      ...
      if (step === 2) {
        ...
        try {
          aiCalling = true;
          const systemInstruction = `[Vai Trò & Bối Cảnh - System Role]
Bạn là một Đạo diễn Video Thương mại, Chuyên gia Ánh sáng và là Bậc thầy Prompting cho các AI tạo ảnh/video (Midjourney, Runway, Kling).
Nhiệm vụ của bạn là phân tích một Kịch Bản Gốc (Master Script) liền mạch và bóc tách nó thành các phân cảnh chi tiết, tối ưu hóa tối đa cho thị giác người xem.

[Yêu Cầu Đầu Ra BẮT BUỘC - Output Structure]
Chia kịch bản thành các phân cảnh nối tiếp nhau dưới dạng một mảng JSON hợp lệ. Mỗi đối tượng phân cảnh trong mảng PHẢI có chính xác 7 thuộc tính sau:
- sceneNumber: (number) số thứ tự phân cảnh (bắt đầu từ 1)
- descriptionVi: (string) Mô Tả Cảnh bằng Tiếng Việt. Mô tả chính xác bối cảnh, hành động nhân vật và góc máy súc tích.
- descriptionEn: (string) Detailed scene description in English (setting, character action, and camera style)
- promptImage: (string) Prompt Ảnh (Image Prompt - Tiếng Anh). Viết bằng tiếng Anh, các từ khóa cách nhau bằng dấu phẩy. Phong cách & Thẩm mỹ: Ưu tiên bối cảnh hiện đại, tối giản công nghiệp (industrial minimalist). Đối với các sản phẩm cơ khí, máy móc hoặc thiết bị, hãy lồng ghép các chất liệu cao cấp như lớp hoàn thiện kim loại (metallic finishes) hoặc bề mặt vân carbon (carbon fiber texture) để tăng độ sang trọng. Màu sắc & Điểm nhấn: Sử dụng ánh sáng điện ảnh (cinematic lighting), độ phân giải 8k, photorealistic. Tận dụng mã màu cyan #34b1b3 làm điểm nhấn tinh tế (ví dụ: đèn neon hắt nhẹ, viền nút bấm, hoặc màu nền). Góc máy: Phải đa dạng (Extreme close-up, Macro shot, Wide shot, Low angle) để video không bị nhàm chán.
- promptVideo: (string) Prompt Video (Video Prompt - Tiếng Anh). Câu lệnh dùng để tạo chuyển động từ ảnh. Viết bằng tiếng Anh. Tập trung hoàn toàn vào chuyển động máy quay và hành động của chủ thể (ví dụ: Slow motion macro shot, smooth camera panning, dynamic transition, fluent fluid motion). Tuyệt đối không để chuyển động bị giật cục.
- voiceScriptVi: (string) Lời Thoại bằng Tiếng Việt: Trích xuất nguyên văn hoặc chia nhỏ một cách hợp lý và mượt mà đoạn thoại từ Kịch Bản Gốc tương ứng với phân cảnh này. Đảm bảo chính tả tuyệt đối (Đặc biệt với tiếng Việt): Rà soát kỹ các từ ngữ, bắt buộc phải viết chuẩn xác: "giặt" (không viết là giạt), "thông báo" (không viết là thòng báo), "nghỉ" (không viết là nghí), "năm mới" (không viết là nâm mới).
- voiceScriptEn: (string) Lời Thoại bằng Tiếng Anh: Voice script translation in English corresponding to this scene statement.

Ví dụ định dạng đầu ra:
[
  {
    "sceneNumber": 1,
    "descriptionVi": "Cận cảnh bàn tay chạm vào chiếc máy xay metallic lấp lánh dưới bóng led cyan tinh xảo.",
    "descriptionEn": "Close up camera of hand touching the metallic blender with cyan light highlights.",
    "promptImage": "extreme close up of hand touching premium metallic blender, sleek industrial minimalist design, cinematic lighting, teal neon accents #34b1b3, photorealistic, 8k",
    "promptVideo": "slow motion macro shot, smooth camera panning to show organic texture of surface",
    "voiceScriptVi": "Chào các bạn, hôm nay cùng mình khám phá siêu phẩm này nhé.",
    "voiceScriptEn": "Hello everyone, let's explore this amazing piece together today."
  }
]
...
CHỈ TRẢ VỀ mảng JSON duy nhất nằm trong dấu ngoặc vuông [], không thêm bất kỳ nhận định, giải thích hay lời nói đầu nào bên ngoài JSON.`;
          ...
          const promptText = `[Dữ Liệu Đầu Vào - Inputs]
Kịch bản gốc: ${project.scriptContent || "Không có kịch bản gốc."}
Ngôn ngữ dự án: ${project.aiLanguage}

[Bắt đầu thực hiện]
Hãy đọc kỹ kịch bản gốc và bóc tách thành các phân cảnh hoàn hảo đúng cấu trúc mảng JSON trên. Trả về kết quả JSON ngay lập tức, tuyệt đối không giải thích thêm.`;
      ...
      if (step === 2) {
        ...
        pendingTask.logLines.push({
          timestamp: new Date().toLocaleTimeString(),
          level: "info",
          message: "🎨 Đang tải các prompts tiếng Anh lên trạm render ảnh nghệ thuật..."
        });
      } else if (step === 8) {
        pendingTask.logLines.push({
          timestamp: new Date().toLocaleTimeString(),
          level: "success",
          message: "✨ Đã dệt xong 6 ảnh phong cảnh độ phân giải cao tỉ lệ 1:1."
        });
      }
    }
...
Nhờ ưu điểm ${project.productInfo ? project.productInfo.substring(0, 100) : "giải quyết nhanh gọn mọi vấn đề bất tiện"}, cuộc sống của mình đã trở nên thảnh thơi và dễ chịu hơn rất nhiều. Bản thân mình đã sử dụng thực tế và hoàn toàn bị thuyết phục bởi độ tiện lợi của nó. Các bạn đừng chần chừ nhé, hay bấm chọn ${project.cta} ngay bên dưới để cùng mình trải nghiệm sự khác biệt này nha!`;
        ...
        if (finalScenesContentArray && finalScenesContentArray.length > 0) {
          project.scenes = finalScenesContentArray.map((sc, scIdx) => ({
            ...
            descriptionVi: sc.descriptionVi || `Cảnh tiếp theo trong chuỗi phân cảnh kịch bản ${project.productName}.`,
            descriptionEn: sc.descriptionEn || `Next dynamic visual sequence tracking product ${project.productName}.`,
            promptImage: sc.promptImage || `High-end minimalist industrial product photoshoot, cinematic lighting, teal accents, photorealistic, 8k`,
            promptVideo: sc.promptVideo || `Smooth camera pan, ultra slow motion, cinematic transition`,
            voiceScriptVi: sc.voiceScriptVi || `Lời thoại phân cảnh ${scIdx + 1}`,
            voiceScriptEn: sc.voiceScriptEn || `English audio script of scene ${scIdx + 1}`,
            status: "Completed"
          }));
        } else {
          project.scenes = [
        ...
        // Ensure scene urls are hydrated
        const dummyImages = [
