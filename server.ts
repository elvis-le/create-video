...
Dựa trên các Dữ Liệu Đầu Vào, hãy viết ngay kịch bản toàn cảnh (Master Script). Chỉ trả về nội dung kịch bản, không giải thích gì thêm.`;
      ...
      if (step === 2) {
        ...
        try {
          ...
          const MASTER_PROMPT_V4 = `Bạn là một Đạo diễn TVC Quảng Cáo Thương Mại kiêm Chuyên gia Prompting. Nhiệm vụ của bạn là bóc tách kịch bản thành các phân cảnh, và viết ra các câu lệnh (Prompts) tạo ảnh/video có chất lượng hình ảnh thương mại.

[Dữ Liệu Đầu Vào]
- Kịch bản gốc: ${masterScript}
- Ngôn ngữ Voice: ${language}
- Phong cách Video: ${videoStyle}
- Hình ảnh Sản phẩm Thực tế (Reference Images): Đã được đính kèm trực tiếp trong yêu cầu này. BẠN PHẢI QUAN SÁT CHÚNG.

[Quy Tắc Hình Ảnh BẮT BUỘC (Dành Cho Prompt Ảnh & Video)]
1. Quy Tắc Nhận Diện Sản Phẩm Thực Tế (CRITICAL PRODUCT ACCURACY): Bạn phải phân tích kỹ các bức ảnh sản phẩm được đính kèm. Trích xuất chính xác màu sắc, hình dáng, cấu tạo và chất liệu của sản phẩm. BẮT BUỘC nhúng đoạn mô tả ngoại hình chính xác này vào MỌI "imagePrompt" có xuất hiện sản phẩm. Tuyệt đối không miêu tả sản phẩm bằng các từ chung chung (Ví dụ: Không viết "a mop". Phải miêu tả chi tiết những gì bạn thấy trong ảnh đính kèm như "a metallic mop with a flat rectangular head and cyan plastic accents").
2. Quy Tắc Cảnh Nỗi Đau (Pain Point): Khi mô tả sự khó khăn, TUYỆT ĐỐI KHÔNG dùng từ khóa u ám (CẤM DÙNG: dramatic shadows, dark, gloomy, horror, high contrast). 
3. Ánh Sáng Thương Mại (Commercial Lighting): BẮT BUỘC sử dụng các từ khóa: "lifestyle commercial photography, bright and airy lighting, soft natural light, studio lighting, modern well-lit interior".
4. Màu Sắc Thương Hiệu: Luôn điểm xuyết: "subtle cyan #34b1b3 accents".
5. Quy Tắc Nhân Vật Nhất Quán & Thuần Việt: Nếu có người, BẮT BUỘC phải chỉ định là người Việt Nam (Ví dụ: "a 25-year-old Vietnamese woman"). Giữ nguyên ngoại hình nhân vật ở tất cả các cảnh.
6. Chuyển Động Video: Tập trung vào sự mượt mà ("smooth motion, fluid motion").

[Cấu Trúc Bóc Tách]
Mỗi phân cảnh phải bao gồm ĐÚNG 4 phần:
1. Mô Tả Cảnh (VI): Mô tả bối cảnh ngắn gọn.
2. Prompt Ảnh (EN): Lệnh tiếng Anh tạo ảnh tĩnh. Chứa chi tiết nhận diện sản phẩm lấy từ ảnh mẫu, chi tiết nhân vật (Vietnamese), bối cảnh, ánh sáng, chất liệu và tỷ lệ (8k, photorealistic).
3. Prompt Video (EN): Lệnh tiếng Anh tạo chuyển động máy quay dựa trên hình ảnh.
4. Giọng Đọc (Voice Script): Lời thoại trích từ kịch bản gốc.

Hãy phân tích kịch bản và CÁC HÌNH ẢNH ĐÍNH KÈM ngay bây giờ! Trả về định dạng JSON mảng các đối tượng chứa: { "description", "imagePrompt", "videoPrompt", "voiceScript" }.`;
