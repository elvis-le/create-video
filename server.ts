...
Dựa trên các Dữ Liệu Đầu Vào, hãy viết ngay kịch bản toàn cảnh (Master Script). Chỉ trả về nội dung kịch bản, không giải thích gì thêm.`;
      ...
      if (step === 2) {
        ...
        try {
          ...
          let lightingStyleRule = `2. Ánh Sáng & Bối Cảnh (Photorealistic Commercial): Luôn dùng "lifestyle commercial photography, bright and airy, soft natural light". TUYỆT ĐỐI KHÔNG dùng từ khóa u ám (dark, gloomy, horror).`;

          switch (project.style) {
            case "3d-pixar":
              lightingStyleRule = `2. Ánh Sáng & Phong Cách (3D Pixar Style): BẮT BUỘC dùng các từ khóa phong cách "3D stylized animation, Pixar and Disney style, cute character design, vibrant colors, clay-like smooth texture, cinematic studio lighting". TUYỆT ĐỐI NGHIÊM CẤM dùng các từ khóa như "photorealistic, real human, lifestyle commercial".`;
              break;
            case "2d-anime":
              lightingStyleRule = `2. Ánh Sáng & Phong Cách (2D Anime Style): BẮT BUỘC dùng các từ khóa phong cách "2D anime style, Studio Ghibli inspired, hand-drawn look, detailed background, soft aesthetic coloring, cinematic anime lighting". TUYỆT ĐỐI NGHIÊM CẤM dùng các từ khóa như "3D, photorealistic, real human".`;
              break;
            case "photorealistic":
            default:
              lightingStyleRule = `2. Ánh Sáng & Bối Cảnh (Photorealistic Commercial): Luôn dùng "lifestyle commercial photography, bright and airy, soft natural light". TUYỆT ĐỐI KHÔNG dùng từ khóa u ám (dark, gloomy, horror).`;
              break;
          }

          const imageParts: any[] = [];
          if (project.imageReferences && Array.isArray(project.imageReferences) && project.imageReferences.length > 0) {
            pendingTask.logLines.push({
              timestamp: new Date().toLocaleTimeString(),
              level: "info",
              message: `📸 Tìm thấy ${project.imageReferences.length} ảnh mẫu sản phẩm thực tế. Đang tải ảnh và nén dạng Base64 gửi lên Gemini Multimodal...`
            });

            for (const imgUrl of project.imageReferences) {
              try {
                ...
                if (imgRes.ok) {
                  ...
                  pendingTask.logLines.push({
                    timestamp: new Date().toLocaleTimeString(),
                    level: "warn",
                    message: `⚠️ Không thể tải ảnh: ${imgUrl} (HTTP status ${imgRes.status})`
                  });
                }
              } catch (fetchErr: any) {
                pendingTask.logLines.push({
                  timestamp: new Date().toLocaleTimeString(),
                  level: "warn",
                  message: `⚠️ Gặp lỗi khi tải ảnh mẫu: ${fetchErr.message || fetchErr}`
                });
              }
            }
          }

          const MASTER_PROMPT_V5 = `Bạn là một Đạo diễn TVC Quảng Cáo Thương Mại kiêm Chuyên gia Prompting. Nhiệm vụ của bạn là bóc tách kịch bản thành các phân cảnh, và viết ra các câu lệnh (Prompts) tạo ảnh/video có chất lượng hình ảnh thương mại.

[Dữ Liệu Đầu Vào]
- Kịch bản gốc: ${masterScript}
- Ngôn ngữ Voice: ${language}
- Phong cách Video: ${videoStyle}
- Hình ảnh Sản phẩm Thực tế (Reference Images): Đã được đính kèm trực tiếp. BẠN PHẢI QUAN SÁT CHÚNG.

[Quy Tắc Hình Ảnh & Video BẮT BUỘC]
1. Quy Tắc Nhận Diện Sản Phẩm (CRITICAL PRODUCT ACCURACY): Trích xuất chính xác màu sắc, hình dáng, chất liệu từ ảnh đính kèm (Ví dụ: "metallic mop with coral velvet pad and cyan #34b1b3 accents"). BẮT BUỘC nhúng mô tả này vào CẢ \`imagePrompt\` VÀ \`videoPrompt\`.
${lightingStyleRule}
3. Nhân Vật Thuần Việt: Nếu có người, BẮT BUỘC chỉ định "Vietnamese". Giữ nguyên ngoại hình ở mọi cảnh.
4. QUY TẮC VIDEO PROMPT (IMAGE-TO-VIDEO TỐI ƯU): 
   - Vì video được tạo ra từ ảnh tĩnh, \`videoPrompt\` KHÔNG ĐƯỢC miêu tả lại một bối cảnh hay nhân vật hoàn toàn mới.
...
[Cấu Trúc Bóc Tách]
Trả về mảng JSON chứa các đối tượng có thuộc tính:
- "description" (VI): Bối cảnh.
- "imagePrompt" (EN): Miêu tả siêu chi tiết bối cảnh, nhân vật, và ĐẶC ĐIỂM SẢN PHẨM NHÌN TỪ ẢNH.
- "videoPrompt" (EN): Góc máy camera + Hành động mượt mà của ĐẶC ĐIỂM SẢN PHẨM TRÊN. (Phải khớp logic với imagePrompt).
- "voiceScript": Lời thoại.`;
...
Trả về kết quả JSON ngay lập tức, tuyệt đối không giải thích thêm.`;
          ...
          const promptPayload = imageParts.length > 0
...
app.post("/api/projects", async (req, res) => {
  const { 
    ...
    productInfo, aiLanguage, goal, tone, style, aiExpertRole, cta, targetWordCount, imageReferences, includeDialogue 
  } = req.body;
